...
func (s *Server) upstreamRoute(c *gin.Context) {
	...
	if s.config.ServerMapping.Enabled {
		s.callStartAPI(endpointID)
	}
...
func (s *Server) panicRoute(c *gin.Context, err any) {
	s.logger.Error("panic in route", zap.Any("error", err))
	c.JSON(http.StatusInternalServerError, gin.H{"error": "internal server error"})
}

// callStartAPI calls the start API for the given endpoint ID.
func (s *Server) callStartAPI(endpointID string) {
	serverIP, exists := s.config.ServerMapping.GetServerIP(endpointID)
	if !exists {
		s.logger.Warn(
			"no server mapping found for endpoint",
			zap.String("endpoint-id", endpointID),
		)
		return
	}
	port := rand.Intn(1000) + 7000
	client := &http.Client{
		Timeout: 10 * time.Second,
	}
	url := fmt.Sprintf("http://%s:1111/hooks/start?port=%d&endpointId=%s", serverIP, port, endpointID)
	resp, err := client.Get(url)
	if err != nil {
		s.logger.Error(
			"failed to call start API",
			zap.String("endpoint-id", endpointID),
			zap.String("server-ip", serverIP),
			zap.Int("port", port),
			zap.Error(err),
		)
		return
	...
	if resp.StatusCode != http.StatusOK {
		s.logger.Warn(
			"start API returned non-OK status",
			zap.String("endpoint-id", endpointID),
			zap.String("server-ip", serverIP),
			zap.Int("port", port),
