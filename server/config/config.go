...
type UpstreamConfig struct {
	...
	// ServerMapping contains the mapping of endpoint prefixes to server IPs.
	ServerMapping ServerMappingConfig `json:"server_mapping" yaml:"server_mapping"`
}

// ServerMappingConfig contains configuration for server mapping functionality.
type ServerMappingConfig struct {
	// Enabled indicates whether server mapping is enabled.
	Enabled bool `json:"enabled" yaml:"enabled"`

	// MappingFile is the path to the server mapping file.
	MappingFile string `json:"mapping_file" yaml:"mapping_file"`

	// ServerMappings contains the mapping of endpoint prefixes to server IPs.
	ServerMappings map[string]string `json:"server_mappings" yaml:"server_mappings"`
}

func (c *ServerMappingConfig) RegisterFlags(fs *pflag.FlagSet, prefix string) {
	if prefix == "" {
		prefix = "server-mapping."
	} else {
		prefix = prefix + ".server-mapping."
	}
	...
	fs.BoolVar(
		...
		prefix+"enabled",
		c.Enabled,
		"Enable server mapping functionality",
	)
	fs.StringVar(
		&c.MappingFile,
		prefix+"mapping-file",
		c.MappingFile,
		"Path to the server mapping file",
	)
}
